...
`features/appearancePreview.ts` persists `{ theme }` to localStorage (per-device) and toggles classes/attributes on `.chat` (`applyAppearance`), read by CSS in `styles/global.css`. `theme` (light/dark) only affects bubble-mode colors via `--bubble-mine-*`/`--bubble-other-*` custom properties, scoped under `.chat--bubbles[data-theme='light']`. The composer (`ChatPage.ts`) is a `<textarea>`, not `<input>`, so messages can carry blank-line paragraph gaps; `utils/linkify.ts` turns URLs/phone numbers into `<a>`/`tel:` links via text-node splitting (same XSS-safe pattern as the existing search highlighter).

**Wallpaper is shared, since 2026-08-27; message style is shared too, since 2026-09-21** — unlike theme, both live on the `connections` row (`wallpaper`, migration 014; `message_style`, migration 034), not localStorage: either member's pick applies to both. `appearancePreview.ts` takes both as params (`applyAppearance(chat, wallpaper, style)`, `openAppearance(anchor, trigger, chat, wallpaper, style, onWallpaperChange, onStyleChange)`) rather than owning them; `ChatPage.ts` applies each optimistically on change via `PATCH /connections/:id/wallpaper` / `PATCH /connections/:id/style` (membership-checked) and re-syncs off the existing 4s connection poll — no new socket event for the poll path. `wallpaper: 'love'` serves `client/public/love.jpg` and overrides the bubble palette (colors pulled from the artwork's own palette) so bubbles read against the art. Wallpaper options are `off` / `love` / `samurai` — a fourth gradient-only option `'1'` was removed 2026-08-31 (migration 023 resets any connection still on it to `off`); style options are `line` / `bubbles`.

**Appearance-change notices, since 2026-09-21** — a real (committed) wallpaper or style change also writes a server-authored `'system'`-type message (migration 034 widens `messages_type_chk`), same trust model as the existing `'call'` type: `socketServer.ts`'s `message:send` rejects a client-sent `'system'` outright, so it can only originate from `messageService.emitAppearanceNotice` (called from the wallpaper/style routes in `routes/connections.ts` when the setter reports the value actually changed — a re-select of the current value is a no-op, no notice). Rapid changes (e.g. scrolling through wallpaper options) are coalesced server-side: `emitAppearanceNotice` debounces per `connectionId+event` for 1.5s, so only the value the burst settles on gets a notice, while the DB column itself still updates on every call. Content is empty; payload is `{event, value}`, encrypted at rest like every other message payload. `emitAppearanceNotice` reaches the socket layer via its own `ioRef`/`setIo`, the same out-of-band pattern `callService.forceEndCall` already uses to emit from outside a socket handler. Client-side, a `'system'` message renders as a plain `chat__system-line` (the leave-events/"start of your one-on-one" style) via `ChatPage.ts`'s `buildMessageRow`, not a `.chat__message` bubble — no receipt, reaction, reply, or quote affordance exists on it, and `registerMessageRow` skips it entirely, same as `'call'`. The backend enforces this too (2026-09-21 hardening): `reactionService.addReaction`/`removeReaction` and `messageService.assertReplyTargetInConnection` now reject a `'system'`-typed message id with 400 — before this, only the client hid the affordance, and a forged `reaction:add`/`message:send{replyTo}` naming a system-message id would have succeeded server-side.
...
- **Stage 4 — FCM push (branch `capacitor/stage-4-fcm`):** the PWA's Web
  ...
  **Notification icon (since 2026-09-21):** native/FCM notifications rendered
  a generic icon — no `default_notification_icon` manifest meta-data meant
  FCM fell back to a default, and the app's actual launcher mipmap wouldn't
  have worked anyway (Android requires a flat white-on-transparent
  silhouette for status-bar icons, not a full-color image). Added
  `drawable/ic_stat_notify.xml`, a monochrome vector of the two-circle mark,
  wired into that meta-data and into `CallForegroundService` (same bug).
...
- **Stage 5 (branch `capacitor/stage-5-navigation`):** hardware/gesture back,
  ...
  **Single-active-panel registry (2026-09-21):** `state/activePanel.ts`
  complements the `backHandlers.ts` LIFO stack above with a separate
  mutual-exclusion concern — before this, the header `MenuDropdown`, the
  appearance panel (`appearancePreview.ts`), and `ChatPage`'s message
  context popover each tracked their own open/closed state independently,
  so two could be visibly open at once (e.g. the appearance panel staying
  open behind the header menu, since `MenuDropdown`'s `stopPropagation`
  suppressed the appearance panel's own outside-click listener).
  `openPanel(close)` now closes whatever's currently active before
  registering the new one; all three call sites register on open and
  unregister on close, and a single shared Escape listener closes whichever
  one is active. Independent of, and layered on top of, `backHandlers.ts`
  (hardware/gesture back still asks "what's topmost," but at most one panel
  can be topmost now).
...
**First-incoming-call permission layering fix (2026-09-21):** `.call-screen` (`z-index: 60`) and `ensurePermissionRationale`'s `Modal` (`z-index: 50`) meant a first-time callee's mic/camera rationale box rendered *underneath* the full-screen ringing surface — invisible, so accepting appeared to do nothing. `controller.ts` `acceptCall()` now hides the call surface (`screen.hidden = true`) before awaiting the rationale, then restores it if the call is still the same `callId` and still ringing, or declines + resets if it was cancelled/ended meanwhile. `acceptCall()` also gained an `accepting` in-flight guard so repeated taps can't stack rationale modals (each prior stacked modal's stale `Continue` resolved a dead `callId`, threw in `transport.accept`, and left the box on screen). Camera rationale copy now mentions microphone too, since a video call needs both permissions.
