...
## Audio calling (V1, since 2026-09-04 — overrides spec §29, user-confirmed)

Calls are a §29 V1 non-goal; audio calling ships as a deliberate, user-confirmed override, the same category as emoji reactions, `/letter`, and image/voice/file media. Video is a separate follow-on batch — the header video button renders `disabled` until then.
...
**Call log:** a `call` message, written **only** by `callService.resolveCall()` at every resolution path (completed / missed / declined / cancelled / failed), through the normal `saveMessage()` so `content` (empty) and `payload` (`{ kind, outcome, durationSec }`) are AES-256-GCM encrypted at rest like every other message. `senderId` is always the caller; the client derives incoming/outgoing framing by comparing to its own user id. `message:send` rejects a client-sent `type: 'call'` outright — call history can't be forged. `saveMessage`'s empty-content allowance covers `'call'` beside `'alarm'`. SDP/ICE blobs (which carry IP addresses) are relayed opaquely and never persisted or logged; the only thing stored is the call-log row.
