...
**Message reports (since 2026-08-29):** `message_reports` table (`message_id`, `reporter_id`, `reason`, RLS enabled). `POST /api/messages/:id/report` → `reportService.reportMessage`, which re-verifies connection membership via `assertMemberOfMessageConnection` (exported from `reactionService`) before inserting. No broadcast, no in-app moderation UI in V1 — satisfies the Play Store UGC reporting requirement.
